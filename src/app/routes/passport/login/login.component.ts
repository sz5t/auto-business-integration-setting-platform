...
export class UserLoginComponent implements OnInit, OnDestroy {
    ...
    submit() {
        ...
        // 构建用户登录信息
        this._buildOnlineUser(onlineUser);
        ...
        /*
        setTimeout(() => {
            this.apiService.post(APIResource.OnlineUser, onlineUser).toPromise()
                .then(response => {
    ...
    _buildOnlineUser(onlineUser) {
        if (this.type === 0) {
            onlineUser.Identify = this.userName.value;
            onlineUser.Password = Md5.hashStr(this.password.value).toString().toUpperCase();
            ...
            onlineUser.Identify = this.uName.value;
            onlineUser.Password = Md5.hashStr(this.uPassword.value).toString().toUpperCase();
